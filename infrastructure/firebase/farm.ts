...
import {
  addDoc, collection, deleteDoc, doc, DocumentReference, endAt, getDoc,
  getDocs, limit, orderBy, query, startAt, updateDoc, where,
...
class FirebaseFarm implements FarmRepository {
  ...
  async getAllWithDetailedProducts(): Promise<Farm[]> {
    const farmsQuery = query(collection(firestore, 'farms'));
    const snapshot = await getDocs(farmsQuery);
